...
class ReportIO {
  ...
  public function createReport($args) {
    ...
    if (isset($args["postId"])) {
      $query = "insert into report (user_id, post_id, report_timestamp) values (:user, :post, now());";
      $bindings = [];
      $bindings[":user"] = $userId;
      $bindings[":post"] = $args["postId"];
    }
    if (isset($args["commentId"])) {
      $query = "insert into report (user_id, comment_id, report_timestamp) values (:user, :comment, now());";
      $bindings = [];
      $bindings[":user"] = $userId;
