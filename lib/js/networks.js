  ...
  var
    ...
    addNetworkToNetworksList = function(listElement, networkData) {
      ...
      networkElement.addEventListener("click", function(e) {
        selectNetwork(networkData);
        document.dispatchEvent(new CustomEvent("networkSelected", {
          detail: networkData.network_id
        }));
      });
    },
    ...
    listNetworks = function(data) {
      ...
      //Clear list
      for (var i in networkList.childNodes) {
        if (networkList.childNodes[i].id != "new-network" && typeof networkList.childNodes[i] == "Node") {
          networkList.removeChild(networkList.childNodes[i]);
        }
      }
