...
window.eventNet.networks = (function() {
  ...
  var
    ...
    renderNetwork = function(listElement, data) {
      ...
      //Network name
      var networkName = window.eventNet.util.append(networkElement, "h3");
      networkName.innerHTML = data.networkName;

      //Network distance
      var networkDistance = window.eventNet.util.append(networkElement, "h4", [DISTANCE_CLASS]);
      networkDistance.innerHTML = beautifyDistance(parseInt(data.distanceFromUser)) + " away";

      //Network posts
      var networkPosts = window.eventNet.util.append(networkElement, "h4", [NUMBER_OF_POSTS_CLASS]);

      var numberOfPosts = 0;
      if (data.number_of_posts) {
        numberOfPosts = parseInt(data.number_of_posts);
      }
      ...
      if (numberOfPosts > 0) {
        numberOfPostsString += ", <time datetime=\"" + data.most_recent_post + "\"></time>";
      }
    ...
    loadNetworks = function() {
      ...
      var url = "api/v1/networks?limit=1000";
