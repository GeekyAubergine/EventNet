...
window.eventNet.networks = (function() {
  ...
  var
    ...
    addNetworkToNetworksList = function(listElement, networkData) {
      ...
      if (networkData.distanceFromUser >= 1) {
        networkDistance.innerHTML = parseInt(networkData.distanceFromUser) + "km away";
      } else {
        networkDistance.innerHTML = parseInt(networkData.distanceFromUser * 1000) + "m away";
      }
    ...
    listNetworks = function(data) {
      ...
      for (var i in data) {
        data[i].distanceFromUser = distanceBetweenToCoords(parseFloat(data[i].network_latitude), parseFloat(data[i].network_longitude), parseFloat(currentLocation.latitude), parseFloat(currentLocation.longitude));
      }
