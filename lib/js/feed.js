...
window.eventNet.feed = (function() {
  ...
  var
        ...
        window.eventNet.xhr.load({
          ...
          callBacks: {
            load: function(response) {
              var data = JSON.parse(response.target.responseText).data;
              mediaFileIds = mediaFileIds.concat(data);
    ...
    createPost = function(event) {
      ...
      var files = document.getElementById("post-creator-post-file-input").files;
    ...
    createComment = function(postId, commentContent, commentList) {
      ...
      window.eventNet.ui.show(getCommentsListForPostId(postId));
    },
    ...
    renderMediaItem = function(parent, src) {
      ...
      var videoRegex = /\.(ogg|mp4|webm)$/
      if (src.match(imageRegex)) {
        var image = window.eventNet.util.append(parent, "img");
        image.src = src;
      }
      else if (src.match(videoRegex)) {
        var video = window.eventNet.util.append(parent, "video");
        video.innerHTML = "Video is not supported by your browser";

        var videoSource = window.eventNet.util.append(video, "source");
        videoSource.src = src;
        var extension = src.match(videoRegex)[0];
    ...
    renderPost = function(postList, data, appendToEnd) {
      ...
      //Meida Container
      var mediaContainer = window.eventNet.util.append(post, "div");
