  ...
  var
    ...
    renderComments = function(commentList, response) {
      ...
      for (var i in response) {
        ...
        data.timestamp = window.eventNet.util.fixDateFormat(data.timestamp);
    ...
    renderPost = function(postList, data, appendToEnd) {
      ...
      //Meida Container
      var mediaContainer = window.eventNet.util.append(post, "div", ["media-container"]);
    ...
    renderPosts = function(response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        } else if (Date.parse(postsOldestDate) < Date.parse(data.timestamp)) {
